...
impl SlackService {
    ...
    async fn handle_search(&self, response_url: &str, query: &str) -> anyhow::Result<()> {
        ...
        let items: Vec<(String, String)> = results.into_iter().map(|r| (r.title, r.url)).collect();
    ...
    async fn handle_chapters(
        ...
        if !self.slack_client.has_bot_token() {
            let items: Vec<(String, String)> =
                chapters.into_iter().map(|c| (c.title, c.url)).collect();
    ...
    pub async fn post_message(
        ...
        self.slack_client
            .post_message(channel, thread_ts, payload)
            .await
    }
    pub async fn handle_thread_test(
        &self,
        channel_id: &str,
        chapter_url: &str,
    ) -> anyhow::Result<()> {
        ...
        let thread_ts = self
            .slack_client
            .post_message(channel_id, None, json!({ "text":  &chapter.title}))
            .await?;

        for payload in views::chapter_images_view(&chapter.title, &chapter.pages) {
            self.slack_client
                .post_message(channel_id, Some(&thread_ts), payload)
                .await?;
            //self.slack_client.respond_in_thread(response_url, payload).await?;
        }
        self.slack_client
